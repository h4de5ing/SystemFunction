...
object CpuSystem {
    ...
    fun allCpu(): List<CpuPseudo> = cpuPesudos

    fun sampleIdleTime(sampleIntervalMill: Int): Long = 0L

    fun idleTime(): Long {
        var total = 0L
        //idle time 智能调整
        // 当 idle时间未发生变化，且当前cpu执行在低频率上，则认为是full idle
        for (cpuPesudo in allCpu()) total += cpuPesudo.idleTime()
        return total
    }

    fun getCpu(index: Int): CpuPseudo = cpuPesudos[index]
}