...
class ProcPseudo(path: String)  {
    ...
    companion object {
        @JvmStatic
        fun create(path: String): ProcPseudo = ProcPseudo(path)

        @JvmStatic
        fun create(): ProcPseudo = create("/proc/" + android.os.Process.myPid() + "/")
        @JvmStatic
        fun createTask(taskId:Long): ProcPseudo =
            create("/proc/" + android.os.Process.myPid() + "/task/"+taskId)
    }
