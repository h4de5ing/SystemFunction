...
object SplitScreenLauncher14 {
    ...
    fun launchSplitPair(
        ...
        if (!isResolvable(appContext, primary.component) ||
            !isResolvable(appContext, secondary.component)
        ) {
    ...
    private fun waitForForeground(
        ...
        val atms = try { ActivityTaskManager.getInstance() } catch (_: Throwable) { null }
        ...
        val poll = object : Runnable {
            override fun run() {
                ...
                if (inMultiWindow && hasRunningTask(activity.applicationContext, atms, primary)) {
                    Log.i(TAG, "caller entered split with primary: ${primary.flattenToShortString()}")
                    callback(true)
                    return
                }
                if (SystemClock.uptimeMillis() >= deadline) {
