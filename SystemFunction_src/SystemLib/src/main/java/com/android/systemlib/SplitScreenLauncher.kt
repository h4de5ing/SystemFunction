...
object SplitScreenLauncher {
    ...
    private fun newMultiWindowOptions(): ActivityOptions {
        ...
        try {
            ...
            ).invoke(opts, /*WindowConfiguration.WINDOWING_MODE_MULTI_WINDOW*/6)
        } catch (t: Throwable) {
    ...
    private fun waitForForeground(
        ...
        val atms = IActivityTaskManager.Stub.asInterface(ServiceManager.getService("activity_task"))
    ...
    private fun isOnTop(
        context: Context,
        atms: IActivityTaskManager?,
        target: ComponentName,
    ): Boolean {
        try {
            atms?.getTasks(1)?.firstOrNull()?.let { top ->
