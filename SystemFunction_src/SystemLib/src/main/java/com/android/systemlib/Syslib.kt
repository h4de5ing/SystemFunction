 ...
 * 禁用Home键按键
 */
fun setHomeKeyDisabled(context: Context, isDisable: Boolean) {
    setStatusBarInt(context, if (isDisable) DISABLE_HOME else DISABLE_NONE)
}
 ...
 * 禁用最近应用按键
 */
fun setRecentKeyDisable(context: Context, isDisable: Boolean) {
    setStatusBarInt(context, if (isDisable) DISABLE_RECENT else DISABLE_NONE)
}

/**
 * 查询是否禁用最近应用键按键
 */
fun isRecentKeyDisable(context: Context): Boolean {
    var enabled = false
    val service = context.getSystemService("statusbar") as StatusBarManager
    val info = service.getDisableInfo()
    "$info".split(" ").forEach {
 ...
 * 禁用返回键
 */
fun setBackKeyDisable(context: Context, isDisable: Boolean) {
    setStatusBarInt(context, if (isDisable) DISABLE_BACK else DISABLE_NONE)
}
 ...
 * 禁用导航栏
 */
fun setNavigaBarDisable(context: Context, isDisable: Boolean) {
    setStatusBarInt(context, if (isDisable) STATUS_DISABLE_NAVIGATION else DISABLE_NONE)
}

/**
 * 查询是否禁用导航栏
 * //TODO 判断mNavigateHome mRecents 是否都是禁用状态
 */
fun isNavigaBarDisable(context: Context): Boolean {
    var enabled = false
    val service = context.getSystemService("statusbar") as StatusBarManager
    val info = service.getDisableInfo()
    println(info)
 ...
 * 禁用状态
 */
fun setStatusBarDisable(context: Context, isDisable: Boolean) {
    setStatusBarInt(context, if (isDisable) DISABLE_EXPAND else DISABLE_NONE)
}
