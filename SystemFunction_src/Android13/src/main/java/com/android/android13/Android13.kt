    ...
    try {
        ...
        cameraService.addListener(object : ICameraServiceListener.Stub() {
            override fun onStatusChanged(status: Int, cameraId: String) {
                //status -2 占用 1释放
                println("onStatusChanged " + (if (status == -2) "占用" else "释放") + " cameraId:" + cameraId)
            ...
            override fun onPhysicalCameraStatusChanged(
                ...
                println("onPhysicalCameraStatusChanged $status cameraId:$cameraId")
            }

            override fun onTorchStatusChanged(status: Int, cameraId: String) {
                println("onTorchStatusChanged status=\" + status + \" cameraId=\" + cameraId + \",\" + (status == 0 ? cameraId + \" 相机被占用\" : cameraId + \" 相机释放")
                if (cameraId != "2") {
            ...
            override fun onCameraClosed(cameraId: String) {
                println("onCameraClosed  $cameraId")
            }
        })
    } catch (e: Exception) {
        e.printStackTrace()
